...
//printf("record is %s long amp ratio is %lf \n", my_record->name, my_record->long_amplitude);
	...
	if(my_record->long_amplitude <=2 )
	{
			...
			{
				my_record->phase_win[i] = my_record->phase_win[i] / fabs(amp);
				x_phase[i] = my_record->phase_beg + i*my_input->delta;
			}
			for(i=0;i<my_record->npts_long;i++)
			{
				my_record->long_win[i] = my_record->long_win[i] / fabs(amp);
				my_record->long_orig[i] = my_record->long_orig[i] / fabs(amp);
	...
	if(my_record->long_amplitude >2 )
	{
			for(i=0;i<my_record->npts_phase;i++)
			{
				my_record->phase_win[i] = my_record->phase_win[i] / fabs(amp2) *0.5;
				x_phase[i] = my_record->phase_beg + i*my_input->delta;
			}
			for(i=0;i<my_record->npts_long;i++)
			{
				my_record->long_win[i] = my_record->long_win[i] / fabs(amp2) * 0.5;
				my_record->long_orig[i] = my_record->long_orig[i] / fabs(amp2) * 0.5;
				x_long[i] = my_record->long_beg + i*my_input->delta;
			}
	}
