	...
	for(ista = 0; ista < my_input->sta_num;ista++)
	{
		...
		for(i=0;i<npts_phase;i++)
		{
			...
			if( my_record[ista].long_amplitude <=2 ) 
				ES_y[i] = my_record[ista].stretched_ES_win[i] /EW_amp / fabs(my_record[ista].long_amplitude );
			else if (my_record[ista].long_amplitude >2 )
				ES_y[i] = my_record[ista].stretched_ES_win[i] /EW_amp *0.5;
		...
		for(i=0;i<npts_phase;i++)
		{
			ES_x[i] = my_record[ista].phase_beg + i*my_input->delta;
			if(my_record[ista].long_amplitude <= 2)
				ES_y[i] = my_record[ista].stretched_gaussian_win[i] / fabs(my_record[ista].long_amplitude) ; 
			else if(my_record[ista].long_amplitude > 2) 
				ES_y[i] = my_record[ista].stretched_gaussian_win[i] *0.5 ; 
			//ES_y[i] = my_record[ista].stretched_ES_win[i] * my_record[ista].polar_flag;
		}
